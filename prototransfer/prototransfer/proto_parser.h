...
public:
	...
	void process(const std::string &proto_file_dir, const std::string &proto_file_base_name, const std::string &protocol_cpp_path)
	{
		...
		for (auto iter = protoinfos_.begin(); iter != protoinfos_.end(); ++iter) {
			...
			file_header_ss << toCppHeaderCodeString(protocol_cpp_path, proto_file_base_name) << std::endl;
			break;
		}
...
private:
	...
	std::string toCppHeaderCodeString(std::string protocol_location_dir, std::string fileBaseName) const {
		std::stringstream ss;
		ss.clear();
		//  zbluenet/demo/proto/
